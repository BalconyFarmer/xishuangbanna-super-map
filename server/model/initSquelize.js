...
class initSquelize {
    ...
    async init() {
        ...
        // 井盖数据
        this.jinggai = sequelize.define('jinggais', {
            id: {
                type: DataTypes.INTEGER(),
                autoIncrement: true,
                primaryKey: true,
    ...
    async addJinggai(params) {

        const result = await this.jinggai.create(
            {
                name: params.name,
    ...
    async getAllJinggai(params) {
        const result = await this.jinggai.findAll(
            {
                'order': [['id', 'DESC']]
            }
        )
    ...
    async getAllJinggaiByType(type) {
        const result = await this.jinggai.findAll({
            where: {
                type: type
            }
